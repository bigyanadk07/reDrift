...
func _ready() -> void:
	global.current_scene="lv-2_3"
	if global.past_scene=="lv-2.4":
		playerpos( Vector2(1870, 2836))
	else:
		playerpos(Vector2(139,2847))
	randomize()
	for i in range(4):  # Spawn 10 monsters
		spawn_monster_skele(44,653,3168,3669)
		spawn_monster_slime(44,653,3168,3669)
		spawn_monster_skele(894,1960,3352,3669)
		spawn_monster_slime(894,1960,3352,3669)
...
func change_scene_to_Dungeon4():
	if global.transition_scene == true:
		global.transition_scene = false
		if global.current_scene == "lv-2_3": 
			print("Changing scene from lv_2.3 to lv_2.4")
			global.past_scene=global.current_scene
			get_tree().change_scene_to_file("res://scenes/level-2/lv_2_4.tscn")
		else:
...
func spawn_monster_skele(a,b,c,d):
...
func spawn_monster_slime(a,b,c,d):
	for i in range(2):  # Try 100 times to find a valid spawn position
		# Get the navigation region's bounds (for random positioning within it)
		 # Get the axis-aligned bounding box of the navigation region
		# Generate a random position within the bounds
